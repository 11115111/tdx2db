...
# Allow running as `python -m rps.cli.run_daily` from repo root
sys.path.insert(0, str(Path(__file__).parent.parent.parent))

from rps.core.db import get_connection, init_tables
...
def main(
    ...
    if init_history:
        ...
        if not skip_sanxianhong:
            # Sanxianhong depends on rps_stock_daily, so run date-by-date.
            dates_sql = f"""
