...
ranked AS (
    SELECT
        r.*,
        CAST(PERCENT_RANK() OVER (PARTITION BY r.trade_date ORDER BY r.block_pct_5d  NULLS FIRST) * 99 AS INTEGER) AS bkrps5,
        CAST(PERCENT_RANK() OVER (PARTITION BY r.trade_date ORDER BY r.block_pct_10d NULLS FIRST) * 99 AS INTEGER) AS bkrps10,
        CAST(PERCENT_RANK() OVER (PARTITION BY r.trade_date ORDER BY r.block_pct_15d NULLS FIRST) * 99 AS INTEGER) AS bkrps15,
        CAST(PERCENT_RANK() OVER (PARTITION BY r.trade_date ORDER BY r.block_pct_20d NULLS FIRST) * 99 AS INTEGER) AS bkrps20,
        CAST(PERCENT_RANK() OVER (PARTITION BY r.trade_date ORDER BY r.block_pct_20d NULLS FIRST) * 99 AS INTEGER) AS bkrps50
